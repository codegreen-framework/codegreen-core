...
def plot_percentage_clean(df,country,save_fig_path=None):
    ...
    df["percentNonRenewable"] = round(((df["total"]-df["renewableTotal"])/df["total"])*100)
    ...
    for i, (index, row) in enumerate(df.iterrows()):
        ...
        ax.bar(i, non_renewable, bar_width, bottom=renewable,  color=Color['red'],edgecolor=Color["red"])
    ...
    ax.set_title('Energy Generation Breakdown: Renewable and Non-Renewable by Hour ('+country+')')
    # ax.legend()
...
def plot_multiple_percentage_clean(dfs, labels,save_fig_path=None):
    ...
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(15 * num_rows, 5 * num_rows), squeeze=False)
    fig.suptitle('Energy Generation Breakdown: Renewable and Non-Renewable by Hour', fontsize=17, y=1)  # Adjust y for positioning
    ...
    for i, (df, label) in enumerate(zip(dfs, labels)):
        ...
        for index, row in df.iterrows():
            ...
            # Plotting bars for renewable and non-renewable
            ax.bar(index, renewable, bar_width, color=Color["green"], edgecolor=Color["green"])
            ax.bar(index, non_renewable, bar_width, bottom=renewable, color=Color["red"], edgecolor=Color["red"])
