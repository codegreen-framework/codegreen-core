...
class ProductsTableDirective(Directive):
    ...
    def run(self):
        ...
        json_path = os.path.join(
            env.srcdir, "../codegreen_core/utilities/country_list.json"
        )
        ...
        for key in full_data["available"]:
            ...
            data.append(
                {"name": c["country"], "code": key, "source": c["energy_source"]}
            )
        ...
        paragraph += nodes.Text(
            f"The following table is automatically generated from 'codegreen_core.utilities.country_list.json' on {date_str}"
        )
        note += paragraph
        ...
        for country in data:
            ...
            paragraph += nodes.literal(
                text=country["code"]
            )  # Inline code block for the country code
