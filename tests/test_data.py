    ...
    def test_entsoe_generation_data(self):
        ...
        for case in cases:
            ...
            if case["dtype"] == "generation":
                ...
                data_verify["end_date"] = (
                    data_verify["MTU"]
                    .str.split(" - ")
                    .str[1]
                    .str.replace(" (UTC)", "", regex=False)
                )
                data_verify["start_date"] = pd.to_datetime(
                    data_verify["start_date"], format="%d.%m.%Y %H:%M"
                )
                data_verify["end_date"] = pd.to_datetime(
                    data_verify["end_date"], format="%d.%m.%Y %H:%M"
                )
                start_utc = pd.to_datetime(
                    case["start"]
                )  # case["start"].astimezone(pd.Timestamp.now(tz='UTC').tzinfo) if case["start"].tzinfo is None else case["start"]
                end_utc = pd.to_datetime(
                    case["end"]
                )  # case["end"].astimezone(pd.Timestamp.now(tz='UTC').tzinfo) if case["end"].tzinfo is None else case["end"]
                filtered_df = data_verify[
                    (data_verify["start_date"] >= start_utc)
                    & (data_verify["start_date"] < end_utc)
                ]
